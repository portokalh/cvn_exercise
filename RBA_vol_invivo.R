...
# Load the data from Excel file
df_stats <- read_excel("RBA_FILES/input_data/volume.xlsx", sheet = "transposed")
df_stats <- df_stats[, 1:163]

# Define output file
output_xlsx <- paste0("vol_invivo_Regional_Stats_FDR_", today_date, ".xlsx")
...
df_stats <- df_stats %>% left_join(df_master %>% select(`BRUNO`, Treatment), by = c("BRUNO" = "BRUNO"))

# Keep only the first 163 columns
numeric_columns <- df_stats %>% select(where(is.numeric)) %>% colnames()
df_stats <- df_stats %>% select(all_of(c("BRUNO", "Treatment", numeric_columns)))
...
# Define output file
output_xlsx <- paste0("vol_invivo_stats_posthoc_", today_date, ".xlsx")
...
for (region in colnames(df_stats)[3:161]) {
...
output_plot <- paste0("vol_invivo_pos_Violin_Panel_", today_date, ".png")
...
output_plot_largest <- paste0("vol_invivo_Violin_Panel_Largest_", today_date, ".png")
...
# Save all plots in a single panel
panel_plot <- ggarrange(plotlist = plots, ncol = 3, nrow = 3)
output_plot <- paste0("vol_invivo_neg_Violin_Panel_", today_date, ".png")
ggsave(output_plot, plot = panel_plot, width = 16, height = 16, dpi = 300)
...
for (region in selected_significant_regions) {
  ...
  output_plot_region <- paste0("loose/vol_invivo_Violin_", gsub(" ", "", region), "", today_date, ".png")
